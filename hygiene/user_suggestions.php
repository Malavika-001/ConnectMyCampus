...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['suggestion'])) {
    ...
    if ($suggestion !== "" && $location !== "") {
        ...
        ");
        if ($stmt->execute([$user_id, $suggestion, $location])) {
            $success_msg = "✅ Suggestion submitted successfully!";
        } else {
            $error_msg = "❌ Error submitting suggestion.";
        }
    }
}

// ✅ Fetch my suggestions (supports both numeric and college_id users)
$myStmt = $pdo->prepare("
    SELECT s.suggestion, s.location, s.status, s.created_at, s.status_updated_at
    FROM suggestions s
    WHERE s.user_id = ? 
       OR s.user_id = (SELECT u.college_id FROM users u WHERE u.id = ?)
    ORDER BY s.created_at DESC
");
$myStmt->execute([$_SESSION['user_id'], $_SESSION['user_id']]);
$mySuggestions = $myStmt->fetchAll();
$myCount = count($mySuggestions);

// ✅ Fetch community suggestions (exclude this user's)
$current_month = date('Y-m');
$commStmt = $pdo->prepare("
    SELECT s.suggestion, s.location, s.status, s.created_at, s.status_updated_at
    FROM suggestions s
    WHERE 
        (s.user_id <> ? AND s.user_id <> (SELECT u.college_id FROM users u WHERE u.id = ?))
        AND DATE_FORMAT(s.created_at, '%Y-%m') = ?
    ORDER BY s.created_at DESC
");
$commStmt->execute([$_SESSION['user_id'], $_SESSION['user_id'], $current_month]);
$communitySuggestions = $commStmt->fetchAll();
$communityCount = count($communitySuggestions);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<?php if($myCount > 0): foreach($mySuggestions as $r): $statusClass='status-'.$r['status']; ?>
<tr>
...
<table>
<tr><th>Suggestion</th><th>Location</th><th>Status</th><th>Date</th></tr>
<?php if($communityCount > 0): foreach($communitySuggestions as $r): $statusClass='status-'.$r['status']; ?>
<tr>
<td><?=htmlspecialchars($r['suggestion'])?></td>
<td><?=htmlspecialchars($r['location'])?></td>
<td>
